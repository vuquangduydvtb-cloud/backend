...
async function checkAuth() {
  loadStoreSettings(); // Load settings for all users
  ...
  document.getElementById('navUser').innerHTML = `
    <div class="user-menu-btn" onclick="toggleUserMenu()">
      ...
      <div class="user-dropdown" id="userDropdown">
        ...
        <button onclick="openModal('changePassModal')">🔐 Đổi mật khẩu</button>
...
async function loadProducts(reset = false) {
  ...
  // Handle category filter (new/used vs accessories)
  if (activeCategoryFilter && activeCategoryFilter !== 'all') {
    if (activeCategoryFilter === 'accessories') {
      params.append('category', 'accessory');
    } else if (activeCategoryFilter === 'new' || activeCategoryFilter === 'used') {
      params.append('condition', activeCategoryFilter);
    }
  }
  
  if (activePhoneModel && activePhoneModel !== 'all') {
    const modelMap = {
      'iphone17': '17', 'iphone16': '16', 'iphone15': '15',
      'iphone14': '14', 'iphone13': '13', 'iphone12': '12'
    };
    if (modelMap[activePhoneModel]) params.append('model', modelMap[activePhoneModel]);
  }
...
function renderProducts(reset) {
  ...
  products.forEach(p => {
    ...
    card.innerHTML = `
      ...
      ${currentUser?.role === 'admin' ? `<div class="card-admin-overlay">
        <button class="card-admin-edit-btn" onclick="openQuickEditModal(${p.id})">⚙️</button>
        <button class="card-admin-sample-btn" onclick="openAdminSampleForm()">＋</button>
      </div>` : ''}
...
function filterByiPhoneModel(model) {
  activePhoneModel = model;
  // Update active state on chips
  document.querySelectorAll('.iphone-filter-chips .filter-chip').forEach((chip, i) => {
    const models = ['all', 'iphone17', 'iphone16', 'iphone15', 'iphone14', 'iphone13', 'iphone12'];
    chip.classList.toggle('active', models[i] === model);
  });
  loadProducts(true);
  document.getElementById('products').scrollIntoView({ behavior: 'smooth' });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', e => {
    if (!e.target.closest('.search-box')) sugg.classList.remove('visible');
    if (!e.target.closest('.category-dropdown-wrapper')) categoryDropdown.classList.remove('active');
  });
    ...
    <div class="pd-grid">
      ...
      <div class="pd-info">
        ...
        <div class="pd-btns">
          ...
          ${currentUser?.role === 'admin' ? `<button class="btn-secondary" onclick="openAdminInfoModal(${p.id})">✏️ Sửa thông tin</button>` : ''}
        </div>
      </div>
    </div>`;
...
async function openQuickEditModal(id) {
  ...
  document.getElementById('qe_product_id').value = p.id;
  document.getElementById('qe_condition').value = p.condition;
  document.getElementById('qe_stock').value = p.stock;
  document.getElementById('qe_colors').value = (p.colors || []).join(', ');

  openModal('adminQuickEditModal');
}

async function submitQuickEdit(e) {
  e.preventDefault();
  const id = document.getElementById('qe_product_id').value;
  const condition = document.getElementById('qe_condition').value;
  const stock = document.getElementById('qe_stock').value;
  const colors = document.getElementById('qe_colors').value.split(',').map(c => c.trim()).filter(c => c);
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) { errEl.textContent = data.error; errEl.classList.add('visible'); return; }
    closeModal('adminQuickEditModal');
    toast('Cập nhật thành công!', 'success');
    loadProducts(true); // Reload
  } catch { errEl.textContent = 'Lỗi kết nối'; errEl.classList.add('visible'); }
}
...
function updateFooterContact() {
  const phone = storeSettings.store_phone || '0909123456';
  const zalo = storeSettings.store_zalo || '0909123456';
  // Update footer
  const footerPhone = document.querySelector('.main-footer a[href^="tel:"]');
  if (footerPhone) {
    footerPhone.href = `tel:${phone}`;
    footerPhone.textContent = phone.replace(/(\d{4})(\d{3})(\d{3})/, '$1 $2 $3');
  }
  const footerZalo = document.querySelector('.main-footer a[href*="zalo"]');
  if (footerZalo) {
    footerZalo.href = `https://zalo.me/${zalo}`;
    footerZalo.textContent = `Zalo: ${zalo.replace(/(\d{4})(\d{3})(\d{3})/, '$1 $2 $3')}`;
  }
}
...
async function loadBanners() {
  try {
    ...
    if (!banners.length) {
      if (defaultBanner) defaultBanner.style.display = 'flex';
      return;
    }
    if (defaultBanner) defaultBanner.style.display = 'none';
    slider.innerHTML = banners.map((b, i) => `
      <div class="banner-image-slide ${i===0?'active':''}" style="background-image: url('${b.image || ''}')">
    ...
    if (banners.length > 1) {
      let cur = 0;
      setInterval(() => {
        slider.children[cur].classList.remove('active');
        cur = (cur + 1) % banners.length;
        slider.children[cur].classList.add('active');
      }, 5000);
    }
  } catch {}
}
...
function openAdminSampleForm() { openModal('adminSampleModal'); }

async function submitAdminBanner(e) {
  e.preventDefault();
  const title = document.getElementById('ab_title').value;
  const subtitle = document.getElementById('ab_subtitle').value;
  const link = document.getElementById('ab_link').value;
  const image = document.getElementById('ab_image').files[0];
  const errEl = document.getElementById('abError');

  if (!image) { errEl.textContent = 'Vui lòng chọn hình ảnh'; errEl.classList.add('visible'); return; }
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) { errEl.textContent = data.error; errEl.classList.add('visible'); return; }
    closeModal('adminBannerModal');
    toast('Banner đã được thêm!', 'success');
    loadBanners();
  } catch { errEl.textContent = 'Lỗi kết nối'; errEl.classList.add('visible'); }
}

async function submitAdminSample(e) {
  e.preventDefault();
  const name = document.getElementById('as_name').value;
  const desc = document.getElementById('as_desc').value;
  const image = document.getElementById('as_image').files[0];
  ...
  try {
    const res = await fetch(`${API}/admin/sample-images`, {
      method: 'POST', headers: { Authorization: `Bearer ${getToken()}` }, body: formData
    });
    const data = await res.json();
    if (!res.ok) { errEl.textContent = data.error; errEl.classList.add('visible'); return; }
    closeModal('adminSampleModal');
    toast('Ảnh mẫu đã được thêm!', 'success');
    // Có lẽ load sample images nếu có
  } catch { errEl.textContent = 'Lỗi kết nối'; errEl.classList.add('visible'); }
}
...
async function submitAdminProduct(e) {
  ...
  const orig = document.getElementById('ap_orig').value;
  const stock = document.getElementById('ap_stock').value;
  const cat = document.getElementById('ap_cat').value;
  const desc = document.getElementById('ap_desc').value;
  const colors = document.getElementById('ap_colors').value;
  const specsStr = document.getElementById('ap_specs').value.trim();
  const images = document.getElementById('ap_images').files;
  const errEl = document.getElementById('apError');

  if (!name || !price) { errEl.textContent = 'Vui lòng điền tên và giá'; errEl.classList.add('visible'); return; }

  let specs = {};
  try {
    specs = specsStr ? JSON.parse(specsStr) : {};
  } catch {
    errEl.textContent = 'JSON specs không hợp lệ'; errEl.classList.add('visible'); return;
  }
  ...
  formData.append('original_price', orig);
  formData.append('stock', stock);
  ...
  try {
    ...
    toast('Sản phẩm đã được thêm!', 'success');
